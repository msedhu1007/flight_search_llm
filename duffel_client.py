 """Duffel API client for flight search"""
  ...
  class DuffelClient:
      ...
      def _format_offers(self, offers_data: dict, passengers: int, max_budget: Optional[float]
  = None) -> dict:
          ...
          if not offers:
              return {"message": f"No flights found within ${max_budget} per passenger
  budget", "offers": []}
  ...
  def format_duffel_for_display(result: dict) -> str:
      ...
      for i, offer in enumerate(offers[:3], 1):
          ...
          output.append(f"**💵 Price:** {offer['price_per_passenger']} per person |
  {offer['total_price']} total")
          ...
          for j, slice_info in enumerate(offer["slices"], 1):
              ...
              output.append(f"### {slice_type}: {slice_info['origin']} →
  {slice_info['destination']}")
              ...
              if slice_info['stops'] > 0:
                  ...
                  for seg in slice_info["segments"]:
                      ...
                      output.append(f"  - {seg['airline']} {seg['flight_number']}:
  {seg['origin']} → {seg['destination']} ({seg_duration})")
